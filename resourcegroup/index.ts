...
const azureAppSpConfig = require("../azureAppSpConfig.json");

const config = {
  clientId: azureAppSpConfig["clientId"] as string,
  clientSecret: azureAppSpConfig["clientSecret"] as string,
  clientObjectId: azureAppSpConfig["clientObjectId"] as string,
  subscriptionId: azureAppSpConfig["subscriptionId"] as string,
  resourceManagerUrl: azureAppSpConfig["resourceManagerUrl"] as string,
  tenantId: azureAppSpConfig["tenantId"] as string,
  location: azureAppSpConfig["location"] as string,
};
 ...
 */
async function main() {
  const metadata = await fetchArmMetadata(config.resourceManagerUrl);
  ...
  const clientOptions = {
    ...
    endpoint: config.resourceManagerUrl
  };
  ...
  try {
    ...
    console.log(`Found ${resourceGroupCount} resource groups.`);
